...
update
{
    ...
    if (vars.CutsceneIndex >= 0 && vars.CutsceneIndex < widgetCount)
    {
        IntPtr widget = vars.Helper.Deref(vars.GEngine, 0x10A8, 0x38, 0x0, 0x30, 0x338, 0x9E8, 0xB0, 0x30 * vars.CutsceneIndex + 0x20);
        ...
        if (widget != IntPtr.Zero)
        {
            ...
            {
                current.Cutscene = vars.FNameToString(vars.Helper.Read<ulong>(widget, 0x460));
            }
            else
            {
                vars.CutsceneIndex = -1;
        ...
        for (int i = 0; i < widgetCount; i++)
        {
            ...
            if (widget != IntPtr.Zero)
            {
                ...
                if (widgetName.StartsWith("WBP_Cutscene_C"))
                {
                    ...
                    current.Cutscene = vars.FNameToString(vars.Helper.Read<ulong>(widget, 0x460));
                    vars.Log("Found cutscene widget at index " + i);
                }
            }
        }
    }
