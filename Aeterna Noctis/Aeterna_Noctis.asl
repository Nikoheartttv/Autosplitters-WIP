...
init
{
    ...
    {
        vars.Helper["bossWatcher"] = mono.Make<int>("ProgressController", "Progress", "bossesDefeated"); //Working
        vars.Helper["abilitiesWatcher"] = mono.Make<int>("ProgressController", "Progress", "kingAbilities"); //Working
        vars.Helper["weaponsWatcher"] = mono.Make<int>("ProgressController", "Progress", "kingWeapons"); //Working
        vars.Helper["currentProgressWatcher"] = mono.Make<int>("KingVariables", "_progressHUD", mono["ProgressHUD"]["_currentProgress"]); //Working
        vars.Helper["playedTimeWatcher"] = mono.Make<int>("LevelManager", 1, "_instance", "playedTime"); //NOT working
        vars.Helper["pausedWatcher"] = mono.Make<bool>("GameManager", "_instance", "_pauseMenuOpen"); //Working

        // vars.Log((mono["LevelManager"].Static + mono["LevelManager"]["_instance"]).ToString("X"));
        ...
        for (int i = 0; i < vars.settingsArray.GetLength(0); i++)
        {
            ...
            if (parent == "Quests")
            {
                //vars.Helper[name] = mono.Make<int>("QuestManager", "_instance", mono["QuestManager"]["QuestList"], 0x10, 0x20 + (0x8 * value), mono["Quest"]["_currentStageIndex"]); //Not working
                vars.Helper[name] = mono.Make<int>("QuestManager", 1, "_instance", "QuestList", 0x10, 0x20 + (0x8 * value), mono["Quest"]["_currentStageIndex"]); //Working
            }
        }
...
update
{
    ...
    if (old.Scene != current.Scene) vars.Log("Scene Change: " + vars.Helper.Scenes.Active.Index.ToString() + ": " + current.Scene);

    if (current.currentProgressWatcher > current.progress && current.currentProgressWatcher < current.progress + 10)
        current.progress = current.currentProgressWatcher;

    // if  (old.playedTimeWatcher != current.playedTimeWatcher)
    //     vars.Log("Played Time: " + TimeSpan.FromSeconds(current.playedTimeWatcher).ToString(@"hh\:mm\:ss"));
}
// update
// {
...
isLoading
{
    ...
    if (settings["PauseStop"] && current.pausedWatcher) return true;
    if (settings["CutSceneStop"] && vars.Cutscenes.Contains(current.Scene)) return true;
    return string.IsNullOrWhiteSpace(vars.Helper.Scenes.Active.Name) || current.Scene == "LoadingScene" || vars.Helper.Scenes.Count > 1;
}
// isLoading
// {
//     if (settings["UseDevTime"]) return true;
...
gameTime
{
    if (settings["UseDevTime"]) return TimeSpan.FromSeconds(current.playedTimeWatcher);
...
split
{
    for (int i = 0; i < vars.settingsArray.GetLength(0); i++)
    {
        ...
        if  (parent == "Bosses" && current.bossWatcher == old.bossWatcher + value)
        {
        ...
        if  (parent == "Abilities" && current.abilitiesWatcher == old.abilitiesWatcher + value)
        {
            vars.Log("Ability: " + description + " acquired!");
            if (settings[name]) vars.queuedSplits++;
        }

        if  (parent == "Weapons" && current.weaponsWatcher == old.weaponsWatcher + value)
        {
        ...
        if (vars.quests.Contains(parent) &&
            vars.Helper[parent].Old < value && vars.Helper[parent].Current >= value)
        {
